...
class User(db.Model):
    ...
    AadharVerified = db.Column(db.Boolean, default=False)
    PANVerified = db.Column(db.Boolean, default=False)
    MonthlyIncome = db.Column(db.Numeric(10, 2))
    MaritalStatus = db.Column(db.Enum("Single", "Married"), nullable=True)
...
class Product(db.Model):
    ...
    loan_offers = db.relationship("LoanOffer", back_populates="product", cascade="all, delete")
    loan_applications = db.relationship("LoanApplication", back_populates="product", cascade="all, delete")
...
class LoanOffer(db.Model):
    ...
    loan_applications = db.relationship("LoanApplication", back_populates="loan_offer", cascade="all, delete")
...
class LoanApplication(db.Model):
    ...
    UserID = db.Column(db.Integer, db.ForeignKey("users.UserID", ondelete="CASCADE"), nullable=False)
    ProductID = db.Column(db.Integer, db.ForeignKey("products.ProductID", ondelete="CASCADE"), nullable=False)
    LoanOfferID = db.Column(db.Integer, db.ForeignKey("loan_offers.OfferID", ondelete="CASCADE"), nullable=False)
    ...
    notifications = db.relationship("SystemNotification", back_populates="application", cascade="all, delete")
...
class SystemNotification(db.Model):
    ...
    SenderID = db.Column(db.Integer, db.ForeignKey("users.UserID", ondelete="CASCADE"), nullable=False)
    ReceiverID = db.Column(db.Integer, db.ForeignKey("users.UserID", ondelete="CASCADE"), nullable=False)
    ApplicationID = db.Column(db.Integer, db.ForeignKey("loan_applications.ApplicationID", ondelete="CASCADE"), nullable=False)
    Message = db.Column(db.Text, nullable=False)
    Timestamp = db.Column(db.DateTime, default=datetime.utcnow)

    sender = db.relationship("User", foreign_keys=[SenderID], back_populates="sent_notifications")
    receiver = db.relationship("User", foreign_keys=[ReceiverID], back_populates="received_notifications")
